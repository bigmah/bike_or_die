...
/* Where a chunk keeps the ARM registers while it runs.
 *
 * Natively they are locals, loaded on the way in and saved on the way out,
 * and clang keeps them in machine registers in between.
 *
 * RECOMP_MEMREGS leaves them in the state they came from, and every
 * instruction reads and writes them there. That is what the WebAssembly build
 * wants, because of what a chunk is: every instruction in it is a label the
 * dispatch at the top can jump to, so every instruction is a place where each
 * of the nineteen can arrive two ways -- from the instruction before, or
 * freshly loaded by the dispatch. Natively that costs nothing. In wasm the
 * edges out of a br_table cannot be split, so clang gives each register a
 * local of its own at every instruction -- thousands of them in a function --
 * and an instruction that changes one register copies all nineteen into the
 * next instruction's set. An optimising tier sees through most of that; a
 * baseline tier, which is where a phone runs a function this size, does every
 * copy. Two loads and a store are much less.
 */
#ifdef RECOMP_MEMREGS
#define AREGS() ((void)0)
#define ALOAD() ((void)0)
#define SAVE()  ((void)0)
#define r0 (S->r[0])
